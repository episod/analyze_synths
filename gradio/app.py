...
def analyze_single_audio_file(
    ...
    if audio_file is None:
        yield "Please upload an audio file to analyze.", None
        return
    
    try:
        # Yield progress updates
        yield "🔄 Preparing analysis...", None
        ...
        yield "📁 Processing uploaded file...", None
        ...
        yield "🔧 Initializing audio analyzer...", None
        ...
        if analysis_type == "mood_only":
            yield "🎵 Extracting audio features...", None
            ...
            yield "⚙️ Loading analysis modules...", None
            ...
            yield "🔍 Running phase detection analysis...", None
            
            # Phase detection analysis
            yield "🎼 Analyzing audio structure...", None
            
            df = analyzer.analyze_directory()
            
            yield "📊 Processing phase data...", None
            ...
            yield "📋 Formatting phase results...", None
            ...
            yield "🚀 Running comprehensive analysis...", None
            
            # Comprehensive analysis
            yield "🎵 Processing audio features...", None
            
            df = analyzer.analyze_directory()
            
            yield "📄 Generating export files...", None
            
            # Generate comprehensive report - export to temp directory directly
            yield "📝 Creating analysis reports...", None
            ...
            yield "📖 Reading generated reports...", None
            ...
            if export_format == "markdown":
                ...
                if 'report_exports' in export_info and 'comprehensive_report' in export_info['report_exports']:
                    ...
                    if report_path.exists():
                        ...
                        yield result_text, download_path
                    else:
                ...
                if 'report_exports' in export_info and 'json_data' in export_info['report_exports']:
                    ...
                    if json_path.exists():
                        ...
                        yield result_text, str(json_path)
                    else:
                ...
                if 'data_exports' in export_info and 'features' in export_info['data_exports']:
                    ...
                    if csv_path.exists():
                        ...
                        yield result_text, str(csv_path)
                    else:
...
def create_interface():
    ...
    with gr.Blocks(
        ...
        with gr.Tab("Audio Analysis"):
            ...
            # Wire up the analysis function - using generator approach
            analyze_btn.click(
                fn=analyze_single_audio_file,
